...
> **EXECUTION STATUS (2026-07-17):** R1–R4 + D1–D6.5 **DONE & committed on `main`** (suite 91 passed, HEAD `9615ad0`). **D7 in progress; D8–D11 pending.** The **"Execution corrections (2026-07-17)"** section below **supersedes** the D4/D6.5/D7 date+reference mechanism (survivor-row dates, WebSearch references) — follow it, not the original code where they conflict. Full resume context is in the repo `README.md` ("Phase 2" block). **Reference URLs: RESOLVED (2026-07-18)** — `data/decree-urls.json` 63→155; the entire residue is cleared except (a) `621/TCTK-PPCĐ` excluded as a code-only re-code and (b) the 5 genuinely-ambiguous bare codes recorded date-qualified pending `(code+effective_date)` keying — see Execution corrections 4–5 below and journal `2026-07-18.01`.
...
3. **Reference URLs come from thuvienphapluat via WebSearch, cached in `data/decree-urls.json`.** thuvienphapluat blocks direct fetch (Cloudflare 403), and the NSO `nghidinh.json` has no per-row URL column — so the D4 "fetch_decrees url extension via lxml" does **not** apply. Instead: WebSearch `<decree code>` restricted to `thuvienphapluat.vn`, take the confirmed `van-ban/…` result, cache `{code: url}`. This is proven (4/4 canonical decrees resolved). The cited reference set is **~150 distinct decrees** (measured: all 157 changed-row codes tie to real events; only 7 are garbled typos) — a large batch resolved incrementally, with garbled/unindexed codes as the manual residue. **[Superseded on the counts by corrections 4–5 below: the reference residue was fully resolved 2026-07-18 (`decree-urls.json` 63→155); of the 157 changed-row codes, one (`621/TCTK-PPCĐ`) is NOT a real event (correction 4), and 5 bare codes are each two decrees (correction 5). See journal `2026-07-18.01`.]**

4. **`621/TCTK-PPCĐ` is a code-only re-code, NOT a lineage event — exclude it from event discovery + the reference gate.** It's a Tổng cục Thống kê công văn (`Số: 621/TCTK-PPCĐ ngày 31/08/2015`) on **66 rows, all dated 2015-08-31**, ghi_chu uniformly **"Điều chỉnh mã địa bàn hành chính"** — it only re-assigns `mã ĐVHC` for units already created in 2015 by real resolutions (903 etc.). Treat it like the 2004→2005 recode: **append the new code to `gso_codes`, emit no referenced event**. So `classify_change` / `window_events` must skip `TCTK` / "Điều chỉnh mã" rows — **with a test asserting no such row also carries a name/type/parent change** (deliberate skip, not silent drop). It needs **no** `decree-urls.json` entry, and the D11 reference gate must never demand a URL for it. (Journal `2026-07-18.01` Residue D.)

5. **5 bare codes are genuinely two decrees — key the reference by `(code, effective_date)`, not bare code.** `04/11/19/33/34 NQ-CP` each name two distinct decrees on different dates (e.g. `04/NQ-CP` = thị xã Dĩ An/Thuận An `ngày 13/01/2011` Bình Dương **AND** thành phố Cẩm Phả retype `ngày 21/2/2012` Quảng Ninh). `decree_for` already disambiguates the *code* by `(effective_date, name)`, but the **URL attach step keyed by bare code** (`load_decree_urls` → `{code: url}`) would give both events the *same* URL → one is mis-referenced. **The discriminating date is already in the `succ_nghi_dinh` cell** (`"Số: <code> ngày <dd/mm/yyyy>"`), so key the override by `(code, effective_date)` (parse the cell) instead of bare code. The 10 per-event URLs are recorded in `data/decree-urls-residue-2026-07-18.json → residue_c_date_qualified`, pending this keying — do NOT flatten them into `decree-urls.json` under a bare code. (`65/89 NQ-CP` were false positives — single decree, stale second date — already flat-keyed and safe.) (Journal `2026-07-18.01` Residue C.)
